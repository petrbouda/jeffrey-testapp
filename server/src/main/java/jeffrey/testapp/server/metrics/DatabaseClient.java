/*
 * Jeffrey - Java Flight Recorder Analyzer
 * Copyright (C) 2023 Petr Bouda
 *
...
package jeffrey.testapp.server.metrics;
...
public class DatabaseClient {
    ...
    public DatabaseClient(NamedParameterJdbcTemplate delegate, String groupLabel) {
    ...
    public int update(StatementLabel statement, String sql, SqlParameterSource paramSource, KeyHolder generatedKeyHolder) {
        JdbcUpdateEvent event = new JdbcUpdateEvent(statement.name().toLowerCase(), groupLabel);
        event.begin();

        int rows = 0;
        try {
            rows = delegate.update(sql, paramSource, generatedKeyHolder);
            event.end();
        } catch (Exception e) {
            event.isSuccess = false;
            throw e;
